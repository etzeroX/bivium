...
test.each([
  [false, new ChatGptBrowserObservationTimeoutError(5)],
  [true, new ChatGptBrowserObservationTimeoutError(5)],
  [false, new Error("locator.evaluate: Connection closed")],
  [true, new Error("locator.evaluate: Execution context was destroyed, most likely because of a navigation")],
])("primary response disconnect resumes without resending (already completed=%s, %s)", async (completed, fault) => {
  ...
  try {
    const answer = await worker.runBrowserTurn({ traceId: "reattach_fixture", modelId: "gpt-5.6-sol", reasoning: "high", capabilities,
      prepare: async () => ({ text: "task", images: [], release: () => { releases++; } }),
